# node-express-andreas-namespace
...
```
tanzu apps workload create node-express-andreas-namespace -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/node-express-andreas-namespace-source \
  --type web
```
...
```
tanzu apps workload get node-express-andreas-namespace
```
